  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [greetingRes, imageRes] = await Promise.all([
          fetch("/api/generate-greeting", {
            method: "POST",
          ...
          fetch("/api/generate-image", {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ 
              prompt: `中国新年贺卡背景，马年主题，骏马，喜庆红金配色，包含灯笼、烟花元素，风格传统与现代结合，无文字。` 
            }),
          })
        ]);
        ...
        if (imageRes.ok) {
          ...
          console.warn("Image generation failed, using default background");
        }
