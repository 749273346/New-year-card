      ...
      const response = await fetch("https://api.deepseek.com/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "你是一个精通中国传统文化和诗词的AI助手。请为用户生成一段2026马年新年祝福，要求创作一首*七言绝句*（每句七个字，共四句），必须紧扣*铁路/火车/高铁*行业特色以及*马年*元素。诗句中**不要**出现用户姓名，也不要使用藏头诗形式。用户姓名**必须**出现在下方的祝福语（wish）中。整体意境要豪迈大气，体现铁路人的无私奉献。返回JSON格式，包含 poem (数组，每句一行) 和 wish (字符串)。"
            },
            {
              role: "user",
  ...
  const response = await fetch("https://open.bigmodel.cn/api/paas/v4/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
         {
           role: "system",
           content: "你是一个精通中国传统文化和诗词的AI助手。请为用户生成一段2026马年新年祝福，要求创作一首*七言绝句*（每句七个字，共四句），必须紧扣*铁路/火车/高铁*行业特色（如钢轨、风笛、复兴号、时刻表、信号灯、通途等）以及*马年*元素（如龙马精神、快马加鞭、万马奔腾等）。诗句中**绝对不要**出现用户姓名，也不要使用藏头诗形式。用户姓名**必须**出现在下方的祝福语（wish）中。内容要豪迈大气，体现铁路人的奉献与时代的飞速发展。返回JSON格式，包含 poem (数组，每句一行) 和 wish (字符串)。"
         },
         {
           role: "user",
           content: `我的名字是${name}，请为我生成马年铁路主题的新年祝福。`
